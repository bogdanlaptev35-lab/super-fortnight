...
//    Профиль.Имя = "Менеджер";
//    Профиль.Идентификатор = "75fa0ecb-98aa-11df-b54f-e0cb4ed5f655"; 
//    Профиль.Роли.Добавить("ДобавлениеИзменениеОбслуживанияКлиентов"); 
//    Профиль = ОписаниеПрофилей.Добавить();
//    Профиль.Имя = "Специалист";
//    Профиль.Идентификатор = "3f2c4a6e-8b1d-4e7f-9a3c-5b8d2e1f7a4c"; 
//    Профиль.Роли.Добавить("ЧтениеОбслуживанияКлиентов");   
//    Профиль = ОписаниеПрофилей.Добавить();
//    Профиль.Имя = "Бухгалтер";
//    Профиль.Идентификатор = "7b3e1a9c-4d8f-4a2e-9b6c-1f5d3e7a8b2c"; 
//    Профиль.Роли.Добавить("МассовоеСозданиеАктов");    
//    Профиль = ОписаниеПрофилей.Добавить();
//    Профиль.Имя = "КадровикРасчетчик";
//    Профиль.Идентификатор = "9a4c2e8d-3b6f-4a1e-8c7d-2f5a6b3e9d1f"; 
//    Профиль.Роли.Добавить("ДобавлениеИзменениеЗарплатыИОтпусков");
//
//
...
// ВКМ_ДобавленныеОбъекты: начало
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт

	// Профиль "Менеджер ИТ фирмы (ВКМ)"
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Имя           = "ВКМ_МенеджерИТФирмы";
	ОписаниеПрофиля.Идентификатор = "334f6091-20e7-40b0-819e-0e7e3b8fd854";
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Менеджер ИТ фирмы (ВКМ)'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Роли.Добавить("ВКМ_ДобавлениеИзменениеОбслуживанияКлиентов");
	ОписанияПрофилей.Добавить(ОписаниеПрофиля);

	// Профиль "Бухгалтер ИТ фирмы (ВКМ)"
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Имя           = "ВКМ_БухгалтерИТФирмы";
	ОписаниеПрофиля.Идентификатор = "efc08bd9-cd76-4145-9ae9-83facdc83710";
	...
	// Профиль "Кадровик ИТ фирмы (ВКМ)"
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Имя           = "ВКМ_КадровикИТФирмы";
	ОписаниеПрофиля.Идентификатор = "cb60fe31-969f-4f19-a7d6-68f22faf6798";
	ОписаниеПрофиля.Наименование  = НСтр("ru = 'Кадровик ИТ фирмы (ВКМ)'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиля.Роли.Добавить("ВКМ_РаботаСКадрДок");
	ОписанияПрофилей.Добавить(ОписаниеПрофиля);

КонецПроцедуры
// ВКМ_ДобавленныеОбъекты: конец
